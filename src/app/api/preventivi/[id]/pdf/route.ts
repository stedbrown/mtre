...
async function generatePDF(preventivo: any, azienda: any): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.text(`Data emissione: ${formatDate(preventivo.data_emissione || preventivo.created_at)}`, 50, 190);
      doc.text(`Data scadenza: ${formatDate(preventivo.data_scadenza)}`, 50, 210);
