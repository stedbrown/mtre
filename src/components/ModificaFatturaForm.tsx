...
interface ModificaFatturaFormProps {
  fattura: Fattura;
...
export default function ModificaFatturaForm({ fattura, dettagli, clienti, servizi, locale }: ModificaFatturaFormProps) {
  const router = useRouter();
  
  const [formData, setFormData] = useState<Omit<Fattura, 'id' | 'cliente'>>({
  ...
  const [dettagliForm, setDettagliForm] = useState<DettaglioFattura[]>(
    dettagli.length > 0 
      ? dettagli 
  ...
  const rimuoviDettaglio = (index: number) => {
    if (dettagliForm.length === 1) {
      setError('Devi avere almeno un dettaglio nella fattura');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validazione
      if (!formData.numero || !formData.data_emissione || !formData.cliente_id) {
      ...
      const response = await fetch(`/api/fatture/${fattura.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(fatturaPatch),
      });
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Errore durante l'aggiornamento della fattura");
      }
      
      // Ora gestisci i dettagli - prima eliminando quelli esistenti
      const deleteResponse = await fetch(`/api/fatture/${fattura.id}/dettagli`, {
        method: 'DELETE',
      });
      ...
      const createResponse = await fetch(`/api/fatture/${fattura.id}/dettagli`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Reindirizza alla pagina di dettaglio della fattura
      setTimeout(() => {
        router.push(`/${locale}/admin/fatture/${fattura.id}`);
        router.refresh();
      }, 1500);
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {success && (
        <div className="bg-green-50 border-l-4 border-green-400 p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-green-700">Fattura aggiornata con successo! Sarai reindirizzato a breve...</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        <h2 className="text-lg font-medium text-gray-900 mb-4">Informazioni fattura</h2>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium text-gray-900">Dettagli fattura</h2>
          <button
            type="button"
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        ...
        <textarea
          ...
          rows={4}
          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          placeholder="Aggiungi eventuali note alla fattura..."
        />
      </div>
      ...
      <div className="flex justify-end space-x-3">
        <button
          type="button"
          onClick={() => router.push(`/${locale}/admin/fatture/${fattura.id}`)}
